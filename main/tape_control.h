 ...
 *
 * 模拟传统磁带机的快进/快退体验：
 * - 按住快进键 ≥800ms（长按），立即进入变速态并以 2.0x 走带（无 1x 缓冲）
 * - 播放速度逐级加速 (2.0x → 4.0x → 8.0x)，累计 5.0s / 8.0s 进入各档位
 * - 松开快进键，立即恢复正常速度
 * - 短按（<800ms）不进变速态，仅跳 5 秒
 * - 长按进入变速态时先补 ±5 秒基准跳退（R046），再 2x 变速走带，避免"刚过长按倒退反少"的断层
 * - 快退键同理，反向加速播放（所有档位均走向后 seek）
 * - 低档（2x/4x）变调快放（I2S 采样率控制），最高档（8x）跳帧模式
 */
