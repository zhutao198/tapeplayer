...
## R108 [2026-09-04] UI 改版 v2 对齐(低风险): 状态栏重排 + 绿进度条 + 格式行

- 文件: `main/display.cpp`(播放界面 UI 构建 + `ui_show_player` 文本)
- 依据: `docs/ui_preview_v2.html`(用户设计的 UI v2 稿, 320x240@2x 复刻参考图)。
  评估结论: 整体可行, 但磁带区(盒壳/红色轮毂/梯形窗/波形)为中高风险, 分阶段推进。
  **用户拍板: 横向波形频谱动画不要**, 故 R108 仅做低风险视觉对齐部分。
- **改动1 — 进度条绿色**: `bar_prog` indicator 由青色 `0x2dd4bf` 改为设计稿绿 `0x22c55e`。
- **改动2 — 状态栏重排**: 模式词映射设计稿缩写 (`SEQ`=顺序/`LST`=列表循环/`SGL`=单曲),
  尾部加 `NOR` 标, 格式 `PLAY 001/020 SEQ NOR`(全 ASCII, 无乱码风险)。
- **改动3 — 新增格式/品牌行**: 新增 `lbl_fmt`, 置于文件名下方 (y=80), 显示
  `MP3|44KHZ|16bit|0320kbps SQ`(灰色小字)。**当前为静态占位**——真实采样率/码率/
  编码需从 audio_player.cpp 跨模块导出, 留待后续接入 (不属于 R108 视觉对齐目标)。
- **改动4 — 布局下移对齐**: 文件名 `lbl_track` 由 y=52 下移到 y=56, 曲名点阵 canvas
  `TRACK_CANVAS_Y` 同步 52→56, 避免与格式行错位。
- **未做(留待 R109+):** 磁带区换皮(盒壳/红色轮毂位图/梯形窗)、AB 屏对齐、启动屏、
  真实音频格式数据接入、SD/扬声器图标精修。
- 实测(烧录后真机): ✅ 进度条绿色; ✅ 状态栏新格式 `PLAY xxx/xxx SEQ NOR`; ✅ 格式行显示;
  ✅ 曲名下移后仍正常中文; ✅ 菜单/browse/时间/提示不受影响。全部通过。
