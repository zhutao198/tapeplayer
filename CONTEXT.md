...
> **最后更新**：2026-08-25（**🏁 里程碑 v1.0-stable（基于 R082）**：libhelix 根治 PV-MP3 崩溃 + 坏帧跳曲保护 + 采样率逐帧/帧头校正，真机全部 MP3 可播放不崩、语速正常）
...
| R082 | 2026-08-25 | `ebf5fcb` | **修复低采样率 MP3 变快变尖**：不再信 Helix 上报，改为每帧自行解析 MPEG 帧头取真实采样率/声道上报 i2s（`躲避的爱` 24000Hz 等） | ✅ |

> **🏁 里程碑 `v1.0-stable`（2026-08-25，基于 R082）**：libhelix 根治 PV-MP3 崩溃 + 坏帧跳曲保护 + 采样率逐帧/帧头校正，真机全部 MP3 可播放不崩、语速正常。回滚：`git checkout v1.0-stable` / `git checkout R082`。
