...
> **最后更新**：2026-08-26（**🏁 里程碑 v1.0-stable（基于 R087）**：libhelix 根治 PV-MP3 崩溃 + 坏帧跳曲保护 + I2S 时钟按文件真实采样率设置（低采样率 24000Hz 不再变快）+ R084 修栈溢出 + R085 进度条/计时器重叠 + R086 seek rb 重置 + R087 彻底修 pause/resume/seek 跳曲（decoder AEL_IO_OK→AEL_IO_TIMEOUT））
...
> **🏁 里程碑 `v1.0-stable`（2026-08-26，基于 R087）**：libhelix 根治 PV-MP3 崩溃 + 坏帧跳曲保护 + I2S 时钟按文件真实采样率设置（修复低采样率 MP3 变快）+ R084 修栈溢出 + R085 进度条/计时器重叠 + R086 seek rb 重置 + R087 彻底修 pause/resume/seek 跳曲（decoder AEL_IO_OK→AEL_IO_TIMEOUT）。回滚：`git checkout v1.0-stable` / `git checkout R087`。
