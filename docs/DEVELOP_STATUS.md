...
> 最后更新：2026-08-12（R047 评审基线 + R048 已提交 + R049 统一菜单：R049a 框架 / R049b A-B 复读 / R049c 按键提示音+OTA 入口 均已完成并通过编译）
...
| 固件 USB 升级（OTA）| 🔧 R049c 已接入口+确认流 | — | 分区表已配 `partitions_ota.csv`；**R049c** 菜单 `系统`→`固件升级`/`USB 存储` 已接入口与信息屏（先桩后实，本机无 WiFi/USB OTG 固件，暂未实现实际接收）|
| **总进度** | **~2/6** | | |
...
3. ✅ **已完成**：R049 统一设置菜单
   ...
   - R049c 按键提示音（I2S PCM）+ 固件升级/USB 存储入口（信息屏桩）；其余 系统 项（语音播报/EQ/书签）已就位或持久化 stub
