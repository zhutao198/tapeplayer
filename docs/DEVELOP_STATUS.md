...
> 最后更新：2026-08-12（R047 评审基线 + R048 已提交 + R049 统一菜单进行中）
...
| 电池低电告警 | ❌ 未实现 | — | 依赖电量检测 |
| 设置菜单（OLED 导航）| 🔧 R049 进行中 | menu.cpp + display.cpp | 统一菜单框架已落地（长按 STOP 入口 + 通用树模型）；R049a 已含 播放模式/定时关机 toggle，A-B/OTA/提示音/书签管理待续 |
...
3. 🔧 **进行中**：R049 统一设置菜单（已落计划 `PLAN_R049_UNIFIED_MENU.md` + R049a 框架：长按 STOP 入口、播放模式/定时关机 toggle；R049b~e 待续）
