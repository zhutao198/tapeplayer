...
| 功能 | 状态 | 备注 |
|---|---|---|
| 定时关机（15/30/60/90 分钟）| ❌ stub | power_mgmt.cpp 仅接口/占位 |
| A-B 区间复读 | ❌ 未实现 | — |
...
1. **现在**：提交 R048（音量 15 档 + SD 检测 + 硬件 PCB 微调 + 评审报告 v3），打标签
2. **接着**：更新 CONTEXT.md / 开发日志.md（补齐 R047~R048 记录）
3. **真机验证**：首次上电验证音频/显示/变速（当前仅有 build + 代码评审，未真机验证）
4. **量产前**：电量检测、OTA、完整测试
